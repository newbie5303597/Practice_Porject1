...
class IssueORM(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    problem_description = Column(Text, unique=True, index=True)
    occurred_at = Column(DateTime, index=True)
    location = Column(String(255), index=True)
    resubmit = Column(String(50), index=True)
    ...
    developer_analysis = Column(Text, nullable=False)
    developer_solution = Column(Text, nullable=False)
    developer_decision = Column(String(50), nullable=False)
